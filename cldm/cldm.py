...
#     # 설정 파일 로드
#     config_path = 'configs/network_test.yaml'
...
#     # ControlLDM 인스턴스화
#     control_ldm = instantiate_from_config(config['model']).to('cuda')

#     # 모델을 torchsummary에 맞게 래핑
#     class Wrapper(torch.nn.Module):
#         def __init__(self, model):
#             super().__init__()
#             self.model = model

#         def forward(self, image, hint, timesteps, context):
#             cond = {'c_concat': [hint], 'c_crossattn': [context]}
#             return self.model.apply_model(image, timesteps, cond)

#     # 모델 래핑
#     wrapped_model = Wrapper(control_ldm)

#     # 입력 샘플 정의
#     sample_image = torch.randn(1, 3, 512, 384).to('cuda')
#     sample_hint = torch.randn(1, 3, 512, 384).to('cuda')
#     timesteps = torch.randint(0, 1000, (1,), device='cuda')
#     context = torch.randn(1, 768).to('cuda')

#     # 모델 구조 출력
#     summary(wrapped_model, [sample_image, sample_hint, timesteps, context])
